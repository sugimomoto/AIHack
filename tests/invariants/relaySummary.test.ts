...
describe("★お知らせ（日常の連絡）", () => {
  ...
  /**
   * ★品目名は事実そのもの。正しく伝えるほど原文と一致する。
   *   落とすと、何についての話かが相手に伝わらない。
   *   （実測：原文どおりに直したら「ご相談が来ています。」だけになった）
   */
  it("★短い品目名は、原文と一致しても落とさない", () => {
    const raw = "スマホ代と自転車代の領収書の画像を送ってください。合算して折半したいです。";
    const s = summaryFromPayload({ subject: "スマホ代と自転車代の領収書" }, raw);
    expect(s).toContain("スマホ代と自転車代の領収書");
  });
