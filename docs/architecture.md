...
| **Anthropic** | **❌** | **❌** | **公式ドキュメント上は非対応。**`tool_use` パターンで代替 |

> ⚠️ **Anthropic について公式ドキュメントと価格カタログの記載が食い違っている。**
> ドキュメントの対応表は「非対応」とする一方、`/api/pricing` の `supported_parameters` には `response_format` が含まれる。
> **構造化出力が必要な処理に Anthropic を割り当てる場合は、実機で疎通確認すること**（→ S3）。
...
| --- | --- | --- | --- | --- | --- |
| **SMALL** | `openai/gpt-5-nano` | **0.05** | 0.40 | ✅ | 分類タスクは**入力が支配的**。最安かつ構造化出力に対応 |
...
| 処理 | 階層 | 入力 | 出力 | 原価 |
| --- | --- | --- | --- | --- |
| 意図分類 | SMALL | 1,500 | 100 | 0.017円 |
| 提案の構造化 | SMALL | 1,500 | 200 | 0.023円 |
...
| ~~A-02~~ | ~~OrcaRouter の構造化出力（JSON Schema）対応可否~~ | ✅ **解決**：`response_format` で対応。**ただし Anthropic は実機検証が必要**（→ §4.0） |
| **A-06** | **Anthropic モデルの `response_format` 実機検証** | 公式ドキュメントと価格カタログの記載が食い違っている。LARGE に Anthropic を採用する場合に必要 |
| **A-07** | OrcaRouter のワークスペース単位レート制限の具体値 | 公開情報になし。Hacker プランの上限を実測で把握する |
