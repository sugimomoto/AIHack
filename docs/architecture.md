...
| プロダクト名 | **Aida（あいだ）** ※仮称 |
| 作成日 | 2026-08-11 |
| 最終更新 | **2026-08-14**（LARGE の呼び出しが 0 になった／バッチを止めた） |
...
flowchart TB
    ...
    subgraph Run["Cloud Run"]
        ...
        JOB["Cloud Run Jobs<br/>★いまは何もしない"]
    end
...
| バッチ | **Cloud Run Jobs ＋ Cloud Scheduler** | ★**いまは何もしない。**逸脱検知・リマインダーをやめたため<br>（経路は残す。→ functional-design.md §5.6） |
...
| 相手への中立文の生成 | **MEDIUM** | 中 | |
| ~~調停案の生成~~ | ~~**LARGE**~~ | 🚫 **やめた** | 検査は効いていたが、**AI が金額に触れる経路を残さない**（P3） |
| 条項の生成 | **なし** | — | ひな形置換 |
| 金額の算定 | **なし** | — | 算定表参照（決定的） |
| ★**金額の単位換算** | **なし** | — | ★**コードで解釈する**（`parseYen`）。<br>実測で原文「6万円」が `5000`/`50000`/`50000` になった |

> ★★ **LARGE の呼び出しが 0 になった。**
> 取り決め・調停案・金額の3経路すべてから LLM を外したためである。
> **P3（数字と条項を LLM に作らせない）に、例外が無くなった。**
...
> ★ **LARGE は、いまどの経路からも呼ばれない。**
> 原価は SMALL / MEDIUM だけで決まる。
...
> ⚠️ **この 93.9% は代表値ではない。**SMALL 2回だけの計測であり、
> 調停案（LARGE）が含まれていない。**そして、いまは LARGE をどこからも呼ばない。**

#### ★ 実利用に近い計測

対話・取次ぎ・合意形成を一通り通したあとの集計（`/metrics`）。

| 時点 | 呼び出し | CT-1 | CT-4 削減率 | 備考 |
| --- | --- | --- | --- | --- |
| 2026-08-12 | 43回（SMALL 18 / MEDIUM 21 / **LARGE 4**） | 0.42円 | 50.2% | 調停案を含む |
| **2026-08-14** | 累計 778回 | **0.43円** | **59.7%** | ★**LARGE を呼ばなくなった後** |

> ★ **調停案（LARGE）をやめたことは、原価にも効いている。**
> ただし CT-1 が下がりきらないのは、**1メッセージあたりの SMALL/MEDIUM の回数は変わらない**ためである。

> **数字は測る範囲で変わる。**`/metrics` は集計期間を画面に出す。
> 期間を示さない削減率は、意味を持たない。
