...
> ★ **読む先を直した（2026-08-14）。**
>
> `appendException` に到達する経路が無くなっていた
> （`terms` API が作る仮案は `effect: "PERMANENT"` のみ）。
> 一方、相談の帰結は `kind: "ADJUSTMENT"` 側にあり、**読まれていなかった。**
>
> → **「今回だけ」の実体は、いまや相談の帰結である。**
> 「決まったこと」は `listAdjustments`（`kind == "ADJUSTMENT"`）を読む。
>
> ★ ただし **`adjustmentStateOf` が `AGREED` のものだけ**出す。
> 片方が書いただけの控えを「決まったこと」に並べるのは、
> 取り決めでも House Rule でも避けてきた誤りと同じである。
...
| **セッション Cookie** | サインアップ／受諾／サインイン時 | 端末 | 失っても、メールで戻れる |
| `authUid` | ★**サインアップ時（＝ケース作成と同時）** | 横断（`collectionGroup` で引ける） | Auth を消すと辿れない |

### ★ サインアップ必須にした（2026-08-14）

**以前**：匿名でケースが始まり、Cookie だけが手がかりだった。
...
**実測：70ケース中 30ケースが、誰も登録していない状態だった。**

**いま**：本人確認が済んでからケースを作る。

```
メールアドレス → リンク → ★ここで caseId / partyId 発行 ＋ authUid を付ける
```

| | |
| --- | --- |
| `POST /api/cases` | ★**410 で閉じた。**画面から呼ばないだけでなく、API としても |
| `POST /api/auth/signup` | 確認 → ケース作成 → `linkAuthUid` → セッション |
| 招待の受諾 | ★**同じく本人確認が要る。**片側だけ辿れない状態を残さない |
| 辞退 | ★要らない。**断るのに、アカウントを作らせない** |

★ **孤児になるケースが、構造として生まれない。**

> ★ `userId` を別に発行しても、この問題は解けなかった。
> **その userId を持つのも Cookie だから**である。
> 解いたのは「**いつ辿れるようになるか**」を前倒ししたことだった。
