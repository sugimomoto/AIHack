...
## ★ 実機で払った授業料（2026-08-14）

机上のレビューでは出ず、**2人で触って初めて出た**ものである。
どれも「正しいコードが、正しく動かない」形をしている。

### 1. flex の子に `overflow-hidden` を付けたら、`shrink-0` も付ける

★ **flex の子は、`overflow` が `visible` でないと自動の最小高さが 0 になる。**

縦の余白が足りないと、**中身を無視して潰れる。**

実測：お相手からの取次ぎカードが **height 2px**（枠線1px × 2）になり、
**DOM にあるのに、画面では細い横線にしか見えなかった。**
角丸で中を切るために `overflow-hidden` を付けたのが原因だった。
...
> ★ **同じ列に並ぶ兄弟のうち、1つだけが違う挙動をする。**
> 吹き出しは `overflow-hidden` を持たないので潰れず、
> **封書だけが消えた。**「一部だけ出ない」ときは、まず CSS の差分を疑う。

### 2. 高さ 0 の目印に `scrollIntoView` を呼んでも効かない

高さの無い要素は「すでに見えている」と判定されうる。
`behavior: "smooth"` も、途中で再描画が入ると**最後まで走らない**（実測：1秒後も `scrollTop` が 0）。

```ts
// ✗
endRef.current?.scrollIntoView({ behavior: "smooth", block: "end" });
// ✓ 入れ物を直接動かす。判定にも動きにも頼らない
el.scrollTop = el.scrollHeight;
```

★ 書体の読み込みで**あとから背が伸びる**ので、
rAF・120ms・400ms・`document.fonts.ready` の各時点で送る。

### 3. `useEffect` の初回取得だけでは、あとから届いたものが出ない

開いたまま置いたタブは、**永久に古いまま**である。
→ `focus` / `visibilitychange` と、見えているあいだの定期取得。
★ **隠れているタブでは取りに行かない。**

### 4. 「動かない」と言われたら、まず**測る**

この日、私は Cookie の共有・スクロール位置・キャッシュを順に疑い、**どれも外した。**
手元で受け取り側のセッションを作り、`getBoundingClientRect()` を1回見た時点で、
**height 2px** が出て終わった。

> ★ **説明を組み立てる前に、数字を1つ取る。**

### 5. コードのコメントは、実装の証明にならない

`NewConsult.tsx` には「ここから外した」と書いてあったが、**外れていなかった。**
手書きの行を消しただけで、一覧は**マスタから引いていた。**
