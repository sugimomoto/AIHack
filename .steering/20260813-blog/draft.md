...
型は**論点ごとに用意**してあります。実際のものを抜き出すと、こうです。

| 論点 | 取り出す項目 |
| --- | --- |
| **養育費** | `monthlyAmount`（月額・整数）／`payDay`（月末・5日・10日・25日）／<br>`until`（18歳・20歳・22歳の3月・卒業）／`payeeAccount` |
| **面会交流** | `frequency`（月1・月2・週1・その他）／`dayOfWeek`／`weekOfMonth`（1〜5）／<br>`timeRange`（`10:00-17:00` の形） |
| **財産分与** | `method`（一括・清算済・後日）／`payerSide`／`amountYen`／`dueDate` |

★ **多くが `enum` です。**「毎月末日に」も「月末に」も「ラスト」も、
入るのは `LAST_DAY` という**決まった値**になります。
...
この型は、**公正証書の条項ひな形と1対1で対応**しています。
だから、合意した `payload` をそのまま流し込めば条項になります（**LLM を通しません**）。

> ★ 型を決めたのは、抽出のためだけではありません。
> **「この枠に入らないものは、公正証書にならない」**という線引きでもあります。
