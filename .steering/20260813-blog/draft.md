...
振り込んだのに記録していないだけかもしれません。**観測できないことは、書きません。**
...
| 機能 | 画面での扱い |
| --- | --- |
| メールの送信 | 送信ボタンを**無効化**し、「まだご利用いただけません」と明記 |
| ONE_TIME の例外を義務に適用 | 例外は記録されるが、**予定画面の金額には反映されない** |
| 準備モードの画面 | ドメインは完成。画面は未実装 |
