...
さらに横断で「**まだ相手と話していない**」がある。**招待を求めない。**求めた時点でこの層は離脱する。

### 5.0.2 全体像

```mermaid
graph TD
    TOP["/ 入口"] --> START["/start<br/>状況の確認 ＋ 同居"]

    START --> CH["/onboarding/children<br/>お子さん（人数・年齢）"]
    CH --> PRO["/onboarding/profile<br/>年収 → 帯に変換して見せる"]

    PRO --> BR{"状況で分かれる"}
    BR -->|"取り決めがある"| TERMS["/onboarding/terms<br/>いまの取り決めを入力"]
    BR -->|"これから決める"| INV["/onboarding/invite<br/>お相手を招待"]
    BR -->|"まだ話していない"| APP
    ...
    RECV -->|"受諾"| RCH["お子さん・年収<br/>（相手側）"]
    RCH --> APP
    RECV -->|"いまは決めない"| DECL["お伝えしません"]

    APP["/app<br/>対話・合意・予定・原案"]
...
| `/start` | 状況＋同居 | **状況を選ばないと先に進めない** |
| `/onboarding/children` | お子さん | **算定表に必須。**生まれ月まで。名前は任意 |
| `/onboarding/profile` | 年収 | **入力した額と、相手に見える帯を並べて示す** |
| `/onboarding/terms` | いまの取り決め | **相手の確認を求めない。**飛ばせる |
| `/onboarding/invite` | 招待 | **2つの渡し方に主従を作らない** |
...
**算定表は双方の年収で引く。**したがって受諾した側にも、お子さんと年収を伺う必要がある。

```
受諾 → お子さん（既に登録済みなら飛ばす）→ 年収 → /app
```
